...
static REFIT_VOLUME *PickOneESP (ESP_LIST *AllESPs) {
    ...
    REFIT_MENU_SCREEN   InstallMenuSrc = {
        {L"Install RefindPlus", TRUE}, {NULL, FALSE}, 0, NULL, 0, NULL, 0, {NULL, FALSE},
    ...
    if (AllESPs) {
        ...
        if (InstallMenu) {
            AddMenuInfoLineCached (InstallMenu, L"Select a partition and press Enter to install RefindPlus");
    ...
    if (Status == EFI_SUCCESS) {
        ...
        if (NewInfo != NULL) {
            ...
            Status = refit_call4_wrapper(BaseDir->SetInfo,
                FilePtr, &gEfiFileInfoGuid, NewInfoSize, (VOID *) NewInfo
            );
            MyFreePool (NewInfo);
            MyFreePool (FilePtr);
            MyFreePool (Buffer);
...
static EFI_STATUS CopyOneFile (
    ...
    Status = refit_call5_wrapper(SourceDir->Open, SourceDir, &SourceFile, SourceName, EFI_FILE_MODE_READ, 0);
    if (Status == EFI_SUCCESS) {
...
static VOID CreateFallbackCSV (IN EFI_FILE *TargetDir) {
    ...
    Status = refit_call5_wrapper(
        TargetDir->Open, TargetDir, &FilePtr, L"\\EFI\\refindplus\\BOOT.CSV",
        EFI_FILE_MODE_READ | EFI_FILE_MODE_WRITE | EFI_FILE_MODE_CREATE, 0
    );
    if (Status == EFI_SUCCESS) {
        Contents = PoolPrint (
...
//   create arguments.
static EFI_STATUS ConstructBootEntry (
...
static EFI_STATUS SetBootDefault (UINTN BootNum) {
    ...
    if (Status == EFI_SUCCESS) {
        ...
        if (!IsAlreadyFirst) {
            ...
            Status = EfivarSetRaw (&GlobalGuid, L"BootOrder", (CHAR8*) NewBootOrder, j * sizeof (UINT16), TRUE);
...
static UINTN PickOneBootOption (IN BOOT_ENTRY_LIST *Entries, IN OUT UINTN *BootOrderNum) {
    ...
    REFIT_MENU_SCREEN    PickOneMenuSrc = {
        {L"Manage EFI Boot Order", TRUE}, {NULL, FALSE}, 0, NULL, 0, NULL, 0, {NULL, FALSE},
    ...
    if (Entries) {
        ...
        if (PickOneMenu) {
            AddMenuInfoLineCached (PickOneMenu, L"Select an option and press Enter to make it the default or '-' to delete it");
            while (Entries != NULL) {
                CHAR16 *Temp = NULL;
                CHAR16 *Filename = NULL;
...
static EFI_STATUS DeleteInvalidBootEntries (VOID) {
    ...
    if (Status == EFI_SUCCESS) {
        ...
        Status = EfivarSetRaw (
            &GlobalGuid, L"BootOrder", (CHAR8*) NewBootOrder, j * sizeof (UINT16), TRUE
        );
        MyFreePool (NewBootOrder);
        MyFreePool (BootOrder);
    } // if
